    ...
    <svg :viewBox="`0 0 ${W} ${H}`" class="arch-svg">
      <defs>
        <radialGradient id="aa-core" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stop-color="rgba(155,109,255,0.30)" />
          <stop offset="100%" stop-color="rgba(155,109,255,0.06)" />
        </radialGradient>
        <filter id="aa-glow">
      ...
      <ellipse :cx="cx" :cy="cy" rx="95" ry="55" fill="none" stroke="rgba(155,109,255,0.15)" stroke-width="1" stroke-dasharray="6 8" class="orbit" />
      <ellipse :cx="cx" :cy="cy" rx="140" ry="80" fill="none" stroke="rgba(155,109,255,0.12)" stroke-width="1" stroke-dasharray="4 12" class="orbit orbit-slow" />
      ...
      <g :opacity="visible ? 1 : 0" :style="tIn(0.7)">
        <rect x="300" y="335" width="240" height="55" rx="14" fill="rgba(224,82,101,0.10)" stroke="rgba(224,82,101,0.30)" stroke-width="1.5" />
      ...
      <g v-for="(line, i) in inputLines" :key="'il-'+i">
        <line :x1="line.x1" :y1="line.y1" :x2="line.x2" :y2="line.y2"
              ...
              :opacity="visible ? 0.55 : 0" :style="tIn(0.6 + i * 0.1)" />
        <!-- Flowing particle -->
        <circle r="2.5" :fill="line.color" :opacity="visible ? 0.75 : 0" :style="tIn(1)">
      ...
      <g v-for="(line, i) in outputLines" :key="'ol-'+i">
        <line :x1="line.x1" :y1="line.y1" :x2="line.x2" :y2="line.y2"
              :stroke="line.color" stroke-width="1.5" marker-end="url(#aa-arrow)"
              :opacity="visible ? 0.55 : 0" :style="tIn(0.9 + i * 0.1)" />
        <circle r="2.5" :fill="line.color" :opacity="visible ? 0.75 : 0" :style="tIn(1.2)">
      ...
      <line :x1="420" y1="335" :x2="420" :y2="cy + 70"
            stroke="rgba(224,82,101,0.40)" stroke-width="1.5" marker-end="url(#aa-arrow)"
            :opacity="visible ? 0.50 : 0" :style="tIn(0.8)" />
    </svg>
  </div>
</template>
...
const inputs = [
  { id: 'agents', label: 'AGENTS.md', sub: '5 Directives · 10-Step Workflow', x: 15, y: 22, w: 195, h: 72, bg: 'rgba(155,109,255,0.12)', border: 'rgba(155,109,255,0.40)', color: '#9B6DFF', fs: 14 },
  { id: 'schemas', label: 'JSON Schemas', sub: '6.3MB TVM · 3.9MB DOM · 341KB MDR', x: 15, y: 115, w: 195, h: 60, bg: 'rgba(245,197,24,0.10)', border: 'rgba(245,197,24,0.32)', color: '#F5C518' },
  { id: 'templates', label: 'YAML Templates', sub: 'Object scaffolds & examples', x: 15, y: 198, w: 195, h: 55, bg: 'rgba(91,117,217,0.10)', border: 'rgba(91,117,217,0.30)', color: '#5B75D9' },
]

const outputs = [
  { id: 'tvms', icon: '☣️', label: 'TVMs', sub: 'schema-validated YAML', x: 630, y: 35, w: 185, h: 48, bg: 'rgba(224,82,101,0.12)', border: 'rgba(224,82,101,0.32)', color: '#E05265' },
  { id: 'doms', icon: '🎯', label: 'DOMs + Signals', sub: 'with data requirements', x: 630, y: 98, w: 185, h: 48, bg: 'rgba(91,117,217,0.12)', border: 'rgba(91,117,217,0.32)', color: '#5B75D9' },
  { id: 'mdrs', icon: '🚨', label: 'MDRs + Queries', sub: 'KQL, SPL production-ready', x: 630, y: 161, w: 185, h: 48, bg: 'rgba(56,201,122,0.12)', border: 'rgba(56,201,122,0.32)', color: '#38C97A' },
  { id: 'pr', icon: '🔀', label: 'Pull Request', sub: 'Ready for human review', x: 630, y: 230, w: 185, h: 48, bg: 'rgba(56,201,122,0.08)', border: 'rgba(56,201,122,0.28)', color: '#38C97A' },
]

const inputLines = [
  { x1: 210, y1: 58, x2: cx - 70, y2: cy - 30, color: 'rgba(155,109,255,0.55)' },
  { x1: 210, y1: 145, x2: cx - 70, y2: cy, color: 'rgba(245,197,24,0.55)' },
  { x1: 210, y1: 225, x2: cx - 70, y2: cy + 20, color: 'rgba(91,117,217,0.55)' },
]

const outputLines = [
  { x1: cx + 70, y1: cy - 30, x2: 630, y2: 59, color: 'rgba(224,82,101,0.55)' },
  { x1: cx + 70, y1: cy - 10, x2: 630, y2: 122, color: 'rgba(91,117,217,0.55)' },
  { x1: cx + 70, y1: cy + 10, x2: 630, y2: 185, color: 'rgba(56,201,122,0.55)' },
  { x1: cx + 70, y1: cy + 35, x2: 630, y2: 254, color: 'rgba(56,201,122,0.45)' },
]
</script>
